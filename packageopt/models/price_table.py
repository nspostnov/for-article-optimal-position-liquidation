__all__ = ['PriceTable', 'PriceTruncatedTable']
...
class PriceTable:
    def __init__(self):
        ...
        self._column_types = ['bigint not null', 'varchar(20) not null',
                'bigint not null', 'double precision not null']
...
class PriceTruncatedTable:
    def __init__(self, date=None, seccode=None):
        ...
        if date is None and seccode is None:
            ...
            self._table_name = 'price' + '_' + str(date)
        elif date is not None and seccode is not None:
            self._table_name = 'price' + '_' + str(date) + '_' + str(seccode)
        else:
            raise ValueError('Price table name: something went wrong')
        self._constraints = None
        self._key = ['date', 'seccode', 'time']
