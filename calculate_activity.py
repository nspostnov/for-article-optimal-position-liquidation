'''This script calculates the daily money order volume (action = 1, 0)
'''
import contextlib
import config
...
class Main:
    ...
    def calculate(self, key):
        ...
        dailyactivitytablecruder = DailyActivityTableCRUDer(
                dailyactivitytable, connector)
        dailyactivityrepo = DailyActivityRepo(
                dailyactivitytablecruder)
        dailyactivitysolver = DailyActivitySolver(
                orderlogagent, orderlogtable, dailyactivitytable, 
                connector)
        dailyactivityagent = DailyActivityAgent(
                dailyactivityrepo, dailyactivitysolver)

        return dailyactivityagent.get(key)
